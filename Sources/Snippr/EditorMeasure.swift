...
enum PixelColorSampler {
    ...
    static func sample(image: CGImage, at pixel: CGPoint) -> NSColor? {
        guard let buf = PixelBuffer(image: image) else { return nil }
    ...
    static func darkest(
        ...
        var bestLum = 1000
        var best: NSColor?
        for y in max(0, cy - radius)...min(buf.height - 1, cy + radius) {
            for x in max(0, cx - radius)...min(buf.width - 1, cx + radius) {
...
enum ImageResizer {
    ...
    static func scale(_ image: CapturedImage, by factor: CGFloat) -> CapturedImage? {
        ...
        let w = max(1, Int((CGFloat(image.cgImage.width) * factor).rounded()))
        let h = max(1, Int((CGFloat(image.cgImage.height) * factor).rounded()))
